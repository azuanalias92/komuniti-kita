...
export async function onRequestGet({ env, request }: { env: { DB: any }; request: Request }) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'failed_to_fetch_roles' }), { status: 500, headers: { 'content-type': 'application/json' } })
  }
}
...
export async function onRequestPost({ request, env }: { request: Request; env: { DB: any } }) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'failed_to_create_role' }), { status: 500, headers: { 'content-type': 'application/json' } })
  }
}
